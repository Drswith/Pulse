...
- The frame budget is capped to the selected display's `visibleFrame` before the panel is ordered. The all-account rail budget must not make the window larger than the screen and leave even the collapsed rail offscreen. `placePanel()` measures the rail offset against the frame the window actually received.
