...
- **`total_quota` mixes lifetimes.** It sums the plan with bonus packs that each carry their own `quota_detail[].expires_at`, and the single `nextResetAt` belongs to none of them. Per-pack expiry is not read.
